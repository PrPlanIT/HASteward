...
- **P3.2 — recovery assumes primary = authority.** ✅ DONE (guard + guided plan). repair/heal/breaker all clone replicas FROM the primary; the `leader_not_primary` outcome was detected but the repair engine only aborted. Now: **(A) action-layer authority guard** — `planTargeted` REFUSES to heal the instance triage names the authority (`MostAdvanced`) whenever the heal is unsafe, and `--force` CANNOT override it (closes the "`repair --instance <authority> --force` rm -rf's the golden node" hole at the action layer, defense-in-depth beyond triage's refusal). **(B) guided recovery plan** — CNPG triage now emits a `Diagnosis` (`cnpg-authority-not-primary` / `cnpg-split-brain-diverged`) with an ordered, escrow-first rebuild-AROUND-the-authority plan (escrow → relieve WAL if wedged → promote authority → heal the rest from it), rendered via a shared `renderDiagnoses` (galera + cnpg). It prescribes only safe HASteward primitives and names the promotion as the deliberate manual step — see P3.2b.
- **P3.2b — automate the promotion of a non-primary/divergent authority.** ✅ DONE (safe slice) — `repair --promote --instance N`. It validates the choice with the pure, unit-tested `planPromotion` (leader_not_primary → the target MUST be the proven authority; diverged → requires --force to designate the survivor; provable → refuse, nothing to promote / would discard the primary's newer data; undeterminable → refuse until inspected), then — reusing the exact `--unwedge` escrow+proof machinery — escrows the full recovery set, verifies it, and persists a promotion `RecoveryProof`, so the promotion is REVERSIBLE before anything is touched. It then prints the cluster-specific swap runbook and stops (`errPromotePrepared`). `--dry-run` previews. `--promote` requires `--instance` and cannot combine with `--no-escrow`.
- **P3.2c — execute the in-place timeline swap.** ⏳ FOLLOW-UP (the one step `--promote` prints but does not run). CNPG has no single safe command to make a divergent/behind replica the primary — a clean switchover needs a healthy streaming topology, which by definition doesn't hold here. Automating it means a rebuild-based promotion (fence + clear the others, swap, re-clone), which is CNPG-version-fragile; a wrong sequence destroys the authority (the exact failure this whole effort prevents). Deferred deliberately until the sequence is proven against a scratch cluster — then it plugs in after the escrow+proof step `--promote` already performs, gated by that persisted proof.
- **P3.3 — no restore-regression guard.** ✅ DONE. A dump restore OVERWRITES the live primary and re-clones replicas from it, so it could silently discard newer/committed data (the class of event that rewound `-3` onto TL9 from an old `2C/99` point). `restore` now runs triage first and applies `restoreRegressionDecision` — the authority principle on restore TARGETS, mirroring the repair guard: `leader_not_primary` → HARD REFUSE (--force cannot override; the newest data is on a replica the restore would re-clone away); `diverged` → refuse unless --force (operator must adjudicate the survivor); provable-primary / undeterminable → still a REWIND of live data, require --force, and the message names the live timeline/LSN at stake. Triage failing is itself fatal (restoring blind is what the guard prevents). Decision logic is a pure, unit-tested function.
- **P3.4 — a stuck authority isn't auto-relieved.** ✅ DONE. The golden `-2` crash-looped on disk-full WAL for days while `prunewal` REFUSED any non-primary target (`ABORT: not the primary`) — so the tool literally could not relieve the one node it most needed to. Two changes: **(1) prunewal now relieves a trapped non-primary AUTHORITY** — `assertReliefEligible` gates it to the proven authority (or a `--force`-designated survivor in a divergence; a plain replica is refused → re-clone instead), and the prune skips the primary-only replica-caughtup gate (nothing streams from a crash-looping non-primary) while still deleting only WAL older than the target's OWN checkpoint REDO (committed data past the checkpoint is kept). **(2) triage raises `cnpg-authority-wal-trapped`** — an URGENT diagnosis when a data-bearing authority is not-ready on a full disk (`disk_full` or ≥95%), with the WAL-relief remedy. Conservative: unknown disk isn't flagged. **(3) relief keeps a diverged authority FENCED** (`OfflinePVCJob.KeepFenced`, auto-applied for the non-primary/authority relief): the disk-full state is the only thing stopping CNPG from `pg_rewind`-ing the trapped authority onto the stale primary's lineage, so unfencing it after relief would destroy the golden data. `prune-wal` now frees the disk but leaves the authority fenced (reconcile is still re-enabled — cluster invariant preserved) for a controlled promotion; the operator unfences it only as part of that promotion. Discovered live on boundary-postgres before executing the relief.
- **P3.6 — disk-full DEADLOCK recovery (replay + recycle in place).** ✅ DONE. `prune-wal` only deletes PRE-checkpoint WAL — useless for an instance whose disk is full of POST-checkpoint WAL it can't recycle because it's too full to start (a self-sustaining freeze; boundary-postgres-2 sat here for weeks). New `prune-wal --deadlock-recover`: escrow the PVC (auto VolumeSnapshot, `--snapshot-class` or provisioner-matched), fence + isolate, relocate `pg_wal` to ephemeral scratch to free the disk, run a **single-user crash-recovery REPLAY + CHECKPOINT** (applies all committed WAL — no data loss — then recycles), `pg_archivecleanup` the pre-checkpoint deadweight, move the small WAL back. Datadir ends clean (`shut down`), de-bloated, on the **same PV** (nothing deleted or grown). Non-primary authority is KEPT FENCED after (reuses P3.4's `OfflinePVCJob.KeepFenced`). Honors `--dry-run`; refuses to trim if the replay doesn't reach a clean shutdown or reports PANIC/FATAL. Plain `prune-wal` now points at it when it finds nothing pre-checkpoint to trim. Proven by hand on boundary-postgres-2 (99% → 9%, 881 MB WAL → 17 MB, 355 tables intact) before codifying. Unit-tested (sequence orchestration, cutoff derivation, refusal paths).
