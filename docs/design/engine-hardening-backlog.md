...
- **P3.2b — automate the promotion of a non-primary/divergent authority.** ⏳ FOLLOW-UP (the one manual step in P3.2's plan). CNPG has no single safe command to make a divergent/behind replica the primary — a clean switchover needs a healthy streaming topology, which by definition doesn't hold here. Safe automation means orchestrating a rebuild-based promotion (reduce the cluster to the authority's data, re-clone the rest), which is fragile + CNPG-version-specific; a wrong sequence destroys the authority (the exact failure this whole effort prevents). Deferred deliberately: prove the sequence against a scratch cluster first, gate every step behind escrow + a persisted RecoveryProof (like `--unwedge`), and expose `--authority <instance>` so the `diverged` case can be driven once a human has chosen the survivor.
